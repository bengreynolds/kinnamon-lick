...
int maxIdleTimeMain = 10000;
int maxIdleTimeRew = 10000;
...
void loop(){
  ...
  if (rec) {
    if (TrialState != prevTrialState){
      // Serial.println("changedState"+String(TrialState) + String(prevTrialState));
      // delay(500);
      if (TrialState == 0){
        Serial.print("W+");
      }
        ...
        maxIdleTime = maxIdleTimeMain;
        Serial.print("Y+");
        // Serial.println("TrialState1: MAXIDLETIME:");
        // Serial.print(maxIdleTime);
        // delay(500);
      }
...
void ChangeVial(int tarVial) {
  // Serial.println("NewVial");
  if (tarVial <= 0 || tarVial > NUM_VIALS) {
    unsigned long TS = millis()-sTime;
    Serial.print("E"+String(TS)+ "+");   //Error: Vial number out of range
    return;
  }
  moveRevolve(tarVial);  
...
void handleLickEvent(int lickPin, int solenoidPin, int delayTime, int &TrialState, int vialDirection, unsigned long maxIdleTime, unsigned long listenTime) {
  ...
  else if (digitalRead(lickPin) == HIGH) {
