...
class MainFrame(wx.Frame):
    def __init__(self):
        ...
        if not os.path.exists(self.prev_configpath):
            ...
            if os.path.exists(self.configpath):
                ...
                self.expData = self.read_config(self.default_configpath)
    ...
    def OnInitialize(self, event):
        ...
        self.evTime = Value(ctypes.c_int, 0)
        self.com = Value(ctypes.c_int, 0)
        self.is_busy = Value(ctypes.c_int, 0)
        self.dmVal = Value(ctypes.c_int, 0)
        self.drVal = Value(ctypes.c_int, 0)
        self.mimVal = Value(ctypes.c_int, 0)
        self.mirVal = Value(ctypes.c_int, 0)
        self.orVal = Value(ctypes.c_int, 0)
        self.vial = Value(ctypes.c_int, 0)
        self.vialDir = Value(ctypes.c_int, 0)
        self.trialState = Value(ctypes.c_int, 0)
        self.ardq = Queue()
        self.ardq_p2read = Queue()
        self.ard = arduino.arduinoCtrl(self.ardq, self.ardq_p2read, self.com,self.is_busy, self.dmVal, self.drVal, self.mimVal, 
                                       self.mirVal, self.orVal,  self.vial, self.vialDir, self.expData, self.trialState)
        self.ard.start()
       
        result = self.ardq_p2read.get()
    ...
    def onSelectOrientation(self, event):
        ...
        if self.init:
            self.orVal.value = int(self.currOr.split()[-1])
            print(self.orVal.value)
            self.com.value = 6
            while self.com.value != 0:
                time.sleep(0.1)
    ...
    def updateStatus(self, event):
        if self.trialState:
            print("TrialState",self.trialState.value)
            if self.trialState.value == 0:
                self.statusbar.SetStatusText('Waiting for Next Input', 2)
            if self.trialState.value == 1:
                self.statusbar.SetStatusText('Waiting for Main Lick', 2)
            if self.trialState.value == 2:
                self.statusbar.SetStatusText('Waiting for Reward Lick', 2)
        
        if self.vial:
            if self.vialDir:
                if self.vialDir.value == 0:
                    acidity = "Acidic"
                else:
                    acidity = "Basic"                
                self.statusbar.SetStatusText(f'Vial: {self.vial.value}/{acidity}', 1)            
    ...
    def onSelectProtocol(self, event):
        ...
        if self.currProtName:
            ...
            pdf = pd.read_excel(os.path.join(self.protPath,self.currProtName))
        ...
        if currMode =='Manual':
            ...
            for b in self.vialButtons:
                ...
                if self.grid:
                    ...
                    protocol_path = os.path.join(self.protPath, self.currProtName)
                    df.to_excel(protocol_path, index=False)
    ...
    def userInterrupt(self):
        ...
        while True:
            if keyboard.is_pressed("esc") or keyboard.is_pressed("q"):
                print("User Stop Requested! Exiting protocol!")
                self.ardq.put('Stop')
                self.ardq_p2read.get()
                self.userQuit = True 
                break  
    ...
    def lickListener(self, vndx):
        hs = self.ardq_p2read.get()
        if hs != 'NewVial':
            if hs in ["RewardSent", "WrongReward"]:
                print(hs)
                rs = self.ardq_p2read.get()
                if rs == 'NewVial':
                    print('SendingNext')
            elif hs == 'Error':
                self.errorMessage()
    ...
    def onVialChange(self, event, vial=None, assoc=None):
        if event:
            ...
            self.vial.value = int(vbLab)
            self.vialDirect = self.dropdowns[int(vbLab)-1].GetStringSelection()
            if self.vialDirect == 'Acidic':
                self.vialDir.value = 0
            elif self.vialDirect == 'Basic':
                self.vialDir.value = 1
            # if self.rec:
            #     self.vOrder.append[self.vial.value]
            #     self.aOrder.append[self.vialDir.value]
        else:
            if vial is not None:
                self.vial.value = int(vial)
                self.vialDir.value = int(assoc)
                print(self.vial.value)
            else:
    ...
    def resetSerialCom(self):
        self.statusbar.SetStatusText('RESETTING %s' % self.expData["COM"], 3)
        self.ardq.put('Reconnect')
        result = self.ardq_p2read.get()
        self.ardq_p2read.get()
        if result == 'Success':
            self.bStart.Enable(True)
